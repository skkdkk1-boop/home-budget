...
export type ChecklistSetupResult = {
  moveDate: string;
  currentStage: "이사 전" | "이사 당일 준비 중" | "이사 후 정리 중";
  helpArea: "계약/행정" | "업체/예약" | "짐 정리" | "입주 후 정리";
};

type ChecklistQuestionPanelProps = {
  onClose: () => void;
  onComplete: (result: ChecklistSetupResult) => void;
};

const stageOptions: ChecklistSetupResult["currentStage"][] = [
  "이사 전",
  "이사 당일 준비 중",
  "이사 후 정리 중",
];

const helpAreaOptions: ChecklistSetupResult["helpArea"][] = [
  "계약/행정",
  "업체/예약",
  "짐 정리",
  "입주 후 정리",
...
export default function ChecklistQuestionPanel({
  onClose,
  onComplete,
}: ChecklistQuestionPanelProps) {
  const [step, setStep] = useState(0);
  const [moveDate, setMoveDate] = useState("");
  const [currentStage, setCurrentStage] =
    useState<ChecklistSetupResult["currentStage"] | null>(null);
  const [helpArea, setHelpArea] =
    useState<ChecklistSetupResult["helpArea"] | null>(null);
  ...
  const completeSetup = () => {
    if (!moveDate || !currentStage || !helpArea) {
      return;
    }
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold">
            몇 가지 질문으로 체크리스트를 구성해요
          </h3>
          <p className="mt-2 text-sm leading-7 text-stone-600">
            질문에 답하면 결과를 체크리스트 그룹 안에 자연스럽게 정리해 보여줍니다.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-[1.75rem] bg-stone-50 p-5 ring-1 ring-stone-200">
        ...
        {step === 0 ? (
          <div className="mt-3">
            ...
            <p className="mt-2 text-sm leading-7 text-stone-600">
              날짜를 기준으로 준비 흐름을 정리할 수 있게 도와줍니다.
            </p>
            <input
              type="date"
              value={moveDate}
        ...
        {step === 2 ? (
          <div className="mt-3">
            <h4 className="text-xl font-semibold">
              가장 먼저 도움이 필요한 영역은 무엇인가요?
            </h4>
            <p className="mt-2 text-sm leading-7 text-stone-600">
              선택한 영역을 우선적으로 보여드릴게요.
            </p>
